...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Editar Carga Académica</h1>
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.cargas-academicas.update', $cargaAcademica) }}">
                @csrf
                @method('PUT')
                            ...
                            <select class="form-select @error('profesor_id') is-invalid @enderror" 
                                ...
                                @foreach($profesores as $profesor)
                                    <option value="{{ $profesor->id }}" 
                                            {{ old('profesor_id', $cargaAcademica->profesor_id) == $profesor->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('periodo') is-invalid @enderror" 
                                   ...
                                   value="{{ old('periodo', $cargaAcademica->periodo) }}" 
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('estado') is-invalid @enderror" 
                                ...
                                <option value="asignado" {{ old('estado', $cargaAcademica->estado) == 'asignado' ? 'selected' : '' }}>Asignado</option>
                                <option value="pendiente" {{ old('estado', $cargaAcademica->estado) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                                <option value="completado" {{ old('estado', $cargaAcademica->estado) == 'completado' ? 'selected' : '' }}>Completado</option>
                                <option value="cancelado" {{ old('estado', $cargaAcademica->estado) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                            </select>
                            @error('estado')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary">Actualizar Carga Académica</button>
                </div>
            </form>
        </div>
    </div>
